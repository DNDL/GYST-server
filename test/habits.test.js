...
describe('test habit routes', () => {
  ...
  it('can create a habit using /POST', async done => {
    ...
    return request(app)
      .post('/api/v1/habits')
      .send({ owner: user._id, title: 'Noahs habit', frequency: 'Daily', goal: 2, why: 'Cause' })
      .then(res => {
  ...
  it('can update a habit', async done => {
    const user = await User.create({ email: 'test@test.com' });
    const habit = await Habit.create({ owner: user._id, title: 'Test habit1', frequency: 'Weekly', goal: 2, why: 'Cause' });
    return request(app)
